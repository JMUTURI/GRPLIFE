...
public class JpdlModelDrawer {  
   ...
   * @throws IOException 
   */  
  private void drawNode(Map<String, Node> nodes, Graphics2D g2, Font font, Hashtable<String, HistoryActivityInstance> listActivities) throws IOException {  
    for (Node node : nodes.values()) {  
      ...
      if (nodeInfos.get(node.getType()) != null) { 
        String Val =  nodeInfos.get(node.getType()).toString();
        String Mypath = sc.getRealPath("/images/icons/48" + "/" + Val);
        System.out.println("Path "+ Mypath);
        FileInputStream MyStream = new FileInputStream(Mypath);
        /*BufferedImage bi2 = ImageIO.read(getClass().getResourceAsStream(  
            Mypath + "/icons/48/" + nodeInfos.get(node.getType())));  */
        BufferedImage bi2 = ImageIO.read(MyStream);  
