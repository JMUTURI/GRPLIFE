...
public class TicketProcessing {
  ...
  public String findPolicySelected() {
    try{
      ...
      DCIteratorBinding dciter = ADFUtils.findIterator("findPolicyTransactionsIterator");
      ...
      while (rowKeySetIter.hasNext()) 
      {
         ...
         session.setAttribute("policyCode", r.getAttribute("ENDR_POL_CODE"));
         session.setAttribute("policyNumber", r.getAttribute("ENDR_POL_POLICY_NO"));
         session.setAttribute("endorsementCode", r.getAttribute("ENDR_CODE"));
         session.setAttribute("Endorsement", r.getAttribute("BTR_TRANS_CODE"));
         session.setAttribute("transactionNumber", r.getAttribute("ENDR_LTR_TRANS_NO"));
         session.setAttribute("ClientCode", r.getAttribute("PRP_CLNT_CODE"));
         session.setAttribute("productType", r.getAttribute("POL_PROD_TYPE"));
        
          
         String Process = "UW"+ (String)r.getAttribute("BTR_TRANS_CODE");
    ...
    public String findUserSelected() {
     try {
          ...
          while (rowKeySetIter.hasNext()) {
              ...
              Row r = dciter.getCurrentRow();
              session.setAttribute("TaskAssignee", r.getAttribute("USR_USERNAME"));
    ...
    public String AttachTicket(){
      ...
      try{
        ...
        if(Module.equalsIgnoreCase("P")||Module.equalsIgnoreCase("E")){
          status=checkTicketIfExists(session.getAttribute("policyCode").toString(), Module);
          if(status){
          ...
          status=checkTicketIfExists(session.getAttribute("QuoteCode").toString(), Module);
          if(status){
          ...
          status=checkTicketIfExists(session.getAttribute("ClaimNo").toString(), Module);
          if(status){
  ...
  public boolean checkTicketIfExists(String code,String level){
      ...
      try{
        ...
        cst.execute();
        if(cst.getString(1)==null || cst.getString(1).equalsIgnoreCase("N")){
           return false;
        }
        cst.close();
        conn.close();
    ...
    public void SystemModuleListener(ValueChangeEvent valueChangeEvent) {
        ...
        if(valueChangeEvent.getOldValue()!=valueChangeEvent.getNewValue()){
            if(systemModule.getValue()==null){
              ...
              session.setAttribute("TicketModule", (systemModule.getValue().toString()));
